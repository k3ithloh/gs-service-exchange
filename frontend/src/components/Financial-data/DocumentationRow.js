    ...
    <div className='w-full mt-6 mx-auto container' style={{height: '45px'}}>
      ...
      <div className="relative text-gray-600 focus-within:text-gray-400">
        <span className="absolute inset-y-0 right-0 flex items-center px-2">
          <button type="submit" onClick={() =>  copy(props.data)} className="p-1 text-white focus:outline-none focus:shadow-outline">
            {
        ...
        <input type="search" id={props.data} disabled value={props.data} name="q" style={{fontFamily: 'DejaVu Sans Mono', backgroundColor: '#262626', color: 'white'}} className="bg-gray-900 rounded-md pl-10 focus:outline-none focus:bg-white focus:text-gray-900 p-2 w-full rounded-lg" autocomplete="off" />
      </div>
    </div>
  )
}